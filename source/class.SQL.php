 ...
 * @package ASICMS
 * @version 0.1.20141003
 * LastChange: mysql_real_escape statt self::convtxt
 */
class SQL {
	...
	function cmd($connection = 0, $sql = "", $silent = false, $values = array()) {
		...
		foreach ($values as $k=>$v) $sql = str_replace("{".$k."}", mysql_real_escape_string($v,$conn), $sql); 
		//Exploit?
/*
		if (strpos($sql, "---") !== FALSE) {
...
*/
	...
	**/
	...
	function cmdrows($connection = 0, $sql = "", $values = array(), $key = null) {
		...
		foreach ($values as $k=>$v) $sql = str_replace("{".$k."}", mysql_real_escape_string($v, $this->Verbindungsnr($connection)), $sql); 
	...
	function BQSset($arr) {
		foreach ($arr as $key => $value) $fSet2[] = ' `'.$key.'` = "'.mysql_real_escape_string($value,$this->Verbindungsnr(0)).'" ';
